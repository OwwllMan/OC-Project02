...
class REM_Shortcodes
{
	...
	function manage_properties($attrs){
		if (is_user_logged_in()) {
			extract( shortcode_atts( array(
				'role' => 'administrator',
			), $attrs ) );
    ...
    public function shortcode_button(){
    	if (isset($_GET['post']) && get_post_type($_GET['post']) != 'rem_property') {
    ...
    function create_property_frontend(){
        if (isset($_REQUEST) && $_REQUEST != '') {
            ...
            // just for previewing prop
            if (isset($_REQUEST['preview_property'])) {
             	$my_post['post_status'] = 'draft';
            }
            if ( isset($_REQUEST['property_preview_id']) && $_REQUEST['property_preview_id'] != '' ) {
            	$my_post['ID'] = $_REQUEST['property_preview_id'];
            }

            $status = apply_filters( 'rem_property_publish_status', $my_post['post_status'], $current_user_data->ID );
    ...
    function delete_property(){
    	if (isset($_REQUEST['p_id']) && current_user_can('delete_post', $_REQUEST['p_id'])) {
    ...
    function render_registration_field($field){
		...
		<li>
			...
			<?php switch ($field['type']) {
				...
				case 'checkbox': ?>
					<input type="checkbox" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
					<?php break;
    ...
    function render_agent_edit_field($field, $agent_id){
		...
		<li style="padding-right: 7px;">
			...
			<?php switch ($field['type']) {
				case 'text': ?>
					<input type="text" value="<?php echo get_user_meta( $agent_id, $field['key'], true ); ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
					<i class="icon fas fa-pencil-alt"></i>
					<?php break;
				case 'number': ?>
					<input type="number" value="<?php echo get_user_meta( $agent_id, $field['key'], true ); ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
					<i class="icon fas fa-pencil-alt"></i>
					<?php break;
				case 'email':
					$user_info = get_userdata( $agent_id ); ?>
					<input type="email" value="<?php echo $user_info->user_email; ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
					<i class="icon fas fa-pencil-alt"></i>
					<?php break;
				case 'image': ?>
					<input class="img-url" type="text" value="<?php echo get_user_meta( $agent_id, $field['key'], true ); ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
					...
					<?php break;
				case 'textarea': ?>
					<textarea rows="4" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?>><?php echo get_user_meta( $agent_id, $field['key'] , true ); ?></textarea>
					<p><?php echo $field['help']; ?></p>
					<?php break;
    ...
    function render_property_field($field, $value = ''){
		...
		$help_text = (rem_get_option('property_fields_help_text') == 'enable') ? '<span title="'.wp_strip_all_tags($field['help']).'" class="glyphicon glyphicon-question-sign"></span>' : '' ;
		...
		<div class="<?php echo $columns; ?> space-form wrap-<?php echo $field['key']; ?>" data-condition_status="<?php echo $show_condition; ?>" data-condition_bound="<?php echo isset($field['condition_bound']) ? $field['condition_bound'] : 'all' ?>" data-condition='<?php echo json_encode($conditions); ?>'>
			<?php switch ($field['type']) {
				...
				case 'number':
				case 'date': ?>
					<label for="<?php echo $field['key']; ?>" class="label-p-title"><?php echo $field['title']; ?> <?php echo $help_text; ?></label>
					<input id="<?php echo $field['key']; ?>" <?php echo $required; ?> class="form-control" value="<?php echo $default_val; ?>" type="<?php echo $field['type']; ?>" title="<?php echo $field['help']; ?>" name="<?php echo $field['key']; ?>">
					<?php break;
				case 'checkbox': ?>
				...
				case 'upload': ?>
			        <div class="input-group">
			            <textarea <?php echo $required; ?> id="<?php echo $field['key']; ?>" name="<?php echo $field['key']; ?>" class="form-control custom-control place-attachment" rows="2" style="resize:none;"><?php echo stripcslashes($value); ?></textarea>     
					...
					<?php break;
				case 'textarea': ?>
					<label class="label-p-title"><?php echo $field['title']; ?> <?php echo $help_text; ?></label>
				...
				case 'select': ?>
					<label class="label-p-title"><?php echo $field['title']; ?> <?php echo $help_text; ?></label>
					<select <?php echo $required; ?> class="dropdown" data-settings='{"cutOff": 5}' name="<?php echo $field['key']; ?>">
